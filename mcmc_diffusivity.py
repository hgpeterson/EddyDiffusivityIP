...
def log_likelihoods(u, u_star):
    ...
    gamma = np.diag(np.ones(180,)*0.1)  #Example for now
    ...
    # this is reasonable 
    move_llikelihood = 0.5 * np.sum(np.linalg.inv(gamma**2) * (y - model(u_star))**2)
    previous_llikelihood =  0.5 * np.sum(np.linalg.inv(gamma**2) * (y - model(u))**2)
...
def main():
    ...
    # this toy example is, made the cov matrix all 1s. It converges to the correct value
    cov = np.ones((180,180))
